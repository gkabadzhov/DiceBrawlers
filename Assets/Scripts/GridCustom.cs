...
public class GridCustom<TGridObject>
{
    ...
    public GridCustom( int width, int height, float cellSize, Vector3 originPosition, Func<GridCustom<TGridObject>, int, int, TGridObject> createGridObject)
    {
        ...
        for (int x = 0; x < gridArray.GetLength(0); x++)
        {
            for (int y = 0; y < gridArray.GetLength(1); y++)
            {
                ...
                debugTextArray[x,y] = UtilsClass.CreateWorldText(gridArray[x,y].ToString(), null, GetWorldPosition(x,y) + new Vector3(cellSize,0,cellSize) * .5f, 20, Color.white, TextAnchor.MiddleCenter);
    ...
    {
        int x, y;
        Debug.Log("in grid " + worldPosition);
        worldPosition.y = 0;
